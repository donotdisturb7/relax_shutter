...
<template>
    <BaseModal :title="post.id ? 'Update Post' : 'Mon weekend !'"
               v-model="show"
               @hide="closeModal">
        <div class="p-4">
            ...
            <label class="relative w-full mb-4 border-2 border-dashed text-center p-3 block"
                   :class="showExtensionsText ? 'border-amber-500' : ''">
                <input class="absolute inset-0 w-full h-full z-10 opacity-0 cursor-pointer"
                       type="file"
                       multiple
                       @change="onAttachmentChoose">
                <PaperClipIcon class="h-6 w-6 mx-auto"/>
                <span class="block text-gray-500">Selectionner une photo ou video</span>
            </label>

            <textarea class="w-full px-3 py-2 border rounded-lg resize-none"
